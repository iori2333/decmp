  ...
  fn create(
    ...
    if sources.len() != 1 {
      return Err(DecmpError::InvalidArchive(
        "zstd can only compress a single file".to_string(),
      ));
    }
    ...
    if src.is_dir() {
      return Err(DecmpError::InvalidArchive(
        "zstd cannot compress directories; use tar.zst instead".to_string(),
      ));
    }

    let compression_level = level.map(|l| l as i32).unwrap_or(0);
    ...
    let mut encoder = zstd::Encoder::new(out_file, compression_level)?;
...
mod tests {
  ...
  fn test_zstd_roundtrip() {
    ...
    let data = b"Hello, zstd world! This is a test content.";
    let src = write_temp_file(tmp.path(), "test.txt", data);
    let archive = tmp.path().join("test.txt.zst");

    let handler = ZstdHandler;
  ...
  #[test]
  fn test_zstd_extract() {
    let tmp = tempfile::tempdir().unwrap();
    let data = b"Hello, zstd world! Extract test.";
    let src = write_temp_file(tmp.path(), "test.txt", data);
    let archive = tmp.path().join("test.txt.zst");
  ...
  fn test_zstd_compression_levels() {
    ...
    let fast = tmp.path().join("fast.zst");
    handler
      .create(&[src.clone()], &fast, None, Some(1))
      .unwrap();

    let best = tmp.path().join("best.zst");
    handler.create(&[src], &best, None, Some(19)).unwrap();
  ...
  #[test]
  fn test_zstd_empty_file() {
    let tmp = tempfile::tempdir().unwrap();
    let src = write_temp_file(tmp.path(), "empty.txt", b"");
    let archive = tmp.path().join("empty.txt.zst");
