...
impl ArchiveHandler for Bzip2Handler {
  fn list(
    &self,
    path: &Path,
  ...
  fn create(
    ...
    if sources.len() != 1 {
      return Err(DecmpError::InvalidArchive(
        "bzip2 can only compress a single file".to_string(),
      ));
    }
    ...
    if src.is_dir() {
      return Err(DecmpError::InvalidArchive(
        "bzip2 cannot compress directories; use tar.bz2 instead".to_string(),
      ));
    }
    ...
    let mut encoder = BzEncoder::new(out_file, compression);
...
mod tests {
  ...
  fn test_bzip2_roundtrip() {
    ...
    let data = b"Hello, bzip2 world! This is a test content.";
    let src = write_temp_file(tmp.path(), "test.txt", data);
    let archive = tmp.path().join("test.txt.bz2");

    let handler = Bzip2Handler;
  ...
  #[test]
  fn test_bzip2_extract() {
    let tmp = tempfile::tempdir().unwrap();
    let data = b"Hello, bzip2 world! Extract test.";
    let src = write_temp_file(tmp.path(), "test.txt", data);
    let archive = tmp.path().join("test.txt.bz2");
  ...
  fn test_bzip2_list() {
    ...
    assert_eq!(entries[0].method, "bzip2");
  }
  ...
  fn test_bzip2_compression_levels() {
    ...
    let fast = tmp.path().join("fast.bz2");
    handler
      .create(&[src.clone()], &fast, None, Some(1))
      .unwrap();

    let best = tmp.path().join("best.bz2");
  ...
  #[test]
  fn test_bzip2_reject_directory() {
    let tmp = tempfile::tempdir().unwrap();
  ...
  #[test]
  fn test_bzip2_empty_file() {
    let tmp = tempfile::tempdir().unwrap();
    let src = write_temp_file(tmp.path(), "empty.txt", b"");
    let archive = tmp.path().join("empty.txt.bz2");
